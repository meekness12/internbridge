...
const DashboardLayout: React.FC = () => {
  ...
  return (
    <div className="min-h-screen flex flex-col font-sans text-slate-900 bg-[var(--color-bg)]">
      {/* Modern Topbar: Responsive Scaling */}
      <header className="h-[70px] lg:h-[80px] flex items-center justify-center sticky top-0 z-[100] bg-white border-b border-slate-200 shadow-sm w-full font-sans px-4 lg:px-6">
        <div className="max-w-[1280px] w-full flex items-center justify-between">
          ...
          <div className="flex items-center gap-4 lg:gap-8">
            ...
            <Link to="/dashboard" className="flex items-center gap-2 no-underline group">
              <div className="w-8 h-8 lg:w-10 lg:h-10 bg-[var(--color-gold)] rounded-full flex items-center justify-center text-white font-black text-lg lg:text-xl shadow-lg group-hover:scale-105 transition-all">
                i
              </div>
              <span className="text-lg lg:text-xl font-bold tracking-tight text-slate-900">InternLink</span>
            </Link>
          </div>
          ...
          <nav className="hidden lg:flex items-center gap-6 h-full">
            ...
            {/* Profile Interface (Desktop) */}
            <div className="relative">
              <button 
                onClick={() => setIsProfileOpen(!isProfileOpen)}
                className={`flex items-center gap-2 pl-2 pr-1 h-10 rounded-full border transition-all ${
          ...
          {/* Mobile Profile Trigger (Replacing Desktop Nav) */}
          <div className="lg:hidden flex items-center gap-4">
             <div className="relative">
                <button 
                  onClick={() => setIsProfileOpen(!isProfileOpen)}
                  className="w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center bg-slate-50 overflow-hidden"
                >
      ...
      {/* Main Workspace: Adjusted padding for bottom nav */}
      <div className="flex flex-1 justify-center w-full min-h-screen">
        <div className="max-w-[1128px] w-full flex gap-6 pt-6 pb-24 lg:pb-20 px-4">
      ...
      <div className="lg:hidden fixed bottom-0 left-0 right-0 z-[150] bg-white/80 backdrop-blur-xl border-t border-slate-100 flex items-center justify-around px-4 h-[72px] pb-safe shadow-[0_-10px_30px_rgba(0,0,0,0.05)]">
        {navItems.slice(0, 5).map((item, idx) => {
          ...
          return (
            <Link 
              key={idx} 
              to={item.path} 
              className={`flex flex-col items-center justify-center gap-1.5 transition-all text-[10px] font-black uppercase tracking-widest no-underline ${
            ...
            >
              ...
              {isActive && (
                <div className="absolute top-0 w-8 h-[3px] bg-[var(--color-brand)] rounded-b-full shadow-[0_2px_10px_rgba(99,102,241,0.5)]"></div>
              )}
            </Link>
          );
        })}
