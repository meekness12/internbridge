    ...
    <div className="max-w-[1400px] mx-auto animate-fade-in pb-20 px-4">
         ...
         {[
           ...
           <div key={i} className="bg-white rounded-[2.5rem] p-8 flex items-center gap-6 border border-slate-100 shadow-xl shadow-slate-200/20 group hover:shadow-2xl transition-all">
              ...
              <div>
                 ...
                 <div className="text-2xl font-serif font-black text-slate-900 tracking-tight leading-none">{k.value}</div>
              </div>
           </div>
         ))}
      </div>
               ...
               <div className="flex items-center gap-3">
                  {user.status !== 'SUSPENDED' ? (
                    <button 
                      ...
                      className="w-12 h-12 bg-slate-50 text-slate-400 hover:bg-amber-500 hover:text-white rounded-2xl flex items-center justify-center transition-all shadow-sm"
                      title="Suspend User"
                    >
                      <UserX size={20} />
                    </button>
                  ...
                  <button 
                    ...
                    className="w-12 h-12 bg-slate-50 text-slate-400 hover:bg-rose-500 hover:text-white rounded-2xl flex items-center justify-center transition-all shadow-sm"
                    title="Delete User"
                  >
                    <Trash2 size={20} />
                  </button>
          ...
          <div className="w-full max-w-xl bg-white rounded-[3.5rem] shadow-[0_60px_150px_rgba(13,148,136,0.3)] overflow-hidden animate-scale-in relative border border-slate-100">
            ...
            <div className="p-12">
               <div className="flex items-center justify-between mb-10">
                  <div>
                     <h3 className="text-3xl font-serif font-black text-slate-900 italic leading-none">Add New <span className="text-slate-400 not-italic">User</span></h3>
                     <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-2">New User Registration Form</p>
                  </div>
                  <button onClick={() => setIsModalOpen(false)} className="w-12 h-12 rounded-2xl bg-slate-50 text-slate-300 hover:text-rose-500 transition-all flex items-center justify-center">
                    <X size={24} />
                  </button>
               </div>
      ...
      {isDeleting && (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-6 bg-slate-900/40 backdrop-blur-2xl animate-fade-in text-center">
          <div className="w-full max-w-md bg-white rounded-[3rem] p-12 shadow-2xl relative overflow-hidden animate-scale-in">
            <div className="w-16 h-16 bg-rose-50 text-rose-500 rounded-2xl flex items-center justify-center mx-auto mb-8 animate-pulse">
               <Trash2 size={32} />
            </div>
            <h3 className="text-3xl font-serif font-black text-slate-900 italic mb-4">Delete <span className="text-rose-600">User</span>?</h3>
            ...
            <div className="flex gap-4">
               <button onClick={() => setIsDeleting(null)} className="flex-1 h-14 bg-slate-50 text-[10px] font-black uppercase tracking-[0.2em] rounded-2xl text-slate-400 hover:bg-slate-100 transition-all">Cancel</button>
