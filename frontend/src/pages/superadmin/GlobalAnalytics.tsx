  ...
  return (
    <div className="max-w-[1400px] mx-auto animate-fade-in pb-20 px-4">
      <PremiumHeader 
        ...
        primaryAction={
          <div className="flex items-center gap-4 px-8 h-14 bg-slate-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-2xl">
             <Calendar size={20} className="text-[var(--accent)]" /> Final Quarter 2024
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
        <div className="lg:col-span-8 bg-[var(--surface)] border border-[var(--border)] rounded-[3.5rem] p-12 shadow-2xl shadow-slate-200/20 group overflow-hidden relative animate-fade-up delay-1">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-16 gap-8 relative z-10">
            <div>
              <h3 className="font-serif text-4xl text-[var(--text)] tracking-tight italic">Placement <span className="text-slate-400 not-italic">Velocity</span></h3>
              <p className="text-[10px] text-slate-300 font-black uppercase tracking-[0.3em] mt-3">Historical Engagement Trend · Global Nodes</p>
            </div>
            <div className="flex bg-[var(--background)] p-2 rounded-2xl border border-[var(--border)] shadow-inner">
              <button className="px-8 py-3 bg-[var(--surface)] text-[var(--text)] shadow-xl rounded-xl text-[10px] font-black uppercase tracking-widest">Monthly</button>
              <button className="px-8 py-3 text-slate-400 hover:text-[var(--text)] rounded-xl text-[10px] font-black uppercase tracking-widest transition-colors">Quarterly</button>
            </div>
          </div>
          
          <div className="h-80 flex items-end justify-between gap-4 px-4 pb-10 border-b-2 border-slate-50 relative z-10">
             {isLoading ? (
               <div className="absolute inset-0 flex items-center justify-center bg-[var(--surface)]/50 backdrop-blur-sm z-20 rounded-[3.5rem]">
               ...
               (data?.placementVelocity || [45, 32, 58, 41, 62, 78, 55, 68, 82, 71, 94, 88].map((v, i) => ({ month: `M${i+1}`, rate: v }))).map((v: { month: string; rate: number }, i) => (
                 <div key={i} className="flex-1 group/bar relative h-full flex flex-col justify-end">
                    <div 
                      className="w-full bg-[var(--accent)] opacity-5 group-hover/bar:opacity-100 transition-all duration-700 rounded-t-xl shadow-glow"
                      style={{ height: `${v.rate}%` }}
                    ></div>
                    <div className="absolute -top-14 left-1/2 -translate-x-1/2 bg-slate-900 text-white font-mono font-black text-[10px] px-4 py-2 rounded-xl opacity-0 group-hover/bar:opacity-100 transition-all shadow-2xl scale-90 group-hover/bar:scale-100 border border-[var(--accent)]/20">
                      {v.rate}%
                    </div>
                    <div className="absolute -bottom-12 left-1/2 -translate-x-1/2 font-mono text-[10px] font-black text-slate-200 uppercase tracking-tighter group-hover/bar:text-[var(--accent)] transition-colors">{v.month}</div>
                 </div>
               ))
             )}
          </div>
          ...
          <div className="mt-24 grid grid-cols-1 md:grid-cols-3 gap-8 relative z-10">
             {[
               ...
               <div key={i} className="p-8 bg-[var(--background)]/50 rounded-[2rem] border border-[var(--border)] hover:bg-[var(--surface)] hover:shadow-2xl transition-all group/stat cursor-pointer">
                  <div className="flex items-center gap-5 mb-6">
                     <div className="w-14 h-14 bg-[var(--surface)] rounded-2xl border border-[var(--border)] flex items-center justify-center text-slate-400 group-hover/stat:bg-slate-900 group-hover/stat:text-[var(--accent)] shadow-inner transition-all">
                        {m.icon}
                     </div>
                     <span className="text-[10px] font-black uppercase text-slate-300 tracking-widest">{m.label}</span>
                  </div>
                  <div className="flex items-end justify-between">
                     <div className="text-3xl font-serif font-black text-[var(--text)]">{m.val}</div>
                     <div className="text-[10px] font-mono font-black text-emerald-500 uppercase tracking-tighter">{m.trend}</div>
                  </div>
               </div>
             ))}
          </div>
          <div className="absolute -bottom-20 -left-20 w-80 h-80 bg-[var(--accent)]/5 rounded-full blur-[100px] opacity-0 group-hover:opacity-100 transition-opacity duration-1000"></div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-10 animate-fade-up delay-2">
           <div className="bg-slate-900 text-white rounded-[3.5rem] p-12 shadow-2xl relative overflow-hidden group">
              <div className="relative z-10">
                <div className="flex justify-between items-start mb-16">
                   <h3 className="font-serif text-4xl tracking-tight leading-none italic">Market <br /><span className="text-[var(--accent)] not-italic opacity-40">Distribution</span></h3>
                   <div className="w-16 h-16 bg-[var(--surface)]/5 rounded-[1.5rem] flex items-center justify-center text-[var(--accent)] border border-white/10 group-hover:rotate-12 transition-transform"><Globe size={32} /></div>
                </div>
                   ...
                   {(data?.sectorDistribution || [
                     ...
                     <div key={i} className="group/item">
                        <div className="flex justify-between items-center mb-4">
                           ...
                           <span className="text-[10px] font-mono font-black text-[var(--accent)]">{cat.percentage}%</span>
                        </div>
                        <div className="h-[6px] w-full bg-[var(--surface)]/5 rounded-full overflow-hidden border border-white/5 shadow-inner">
                ...
                <div className="mt-20 pt-12 border-t border-white/5 flex items-center justify-between">
                   ...
                   <button className="h-12 w-12 bg-[var(--surface)]/5 rounded-2xl flex items-center justify-center text-white/20 hover:text-[var(--accent)] hover:bg-[var(--surface)]/10 transition-all border border-white/10 shadow-2xl shadow-black"><BarChart3 size={24} /></button>
                </div>
              </div>
              {/* Decorative Elements */}
           ...
           <div className="bg-[var(--surface)] rounded-[3.5rem] p-12 border-2 border-dashed border-[var(--border)] group relative overflow-hidden shadow-xl shadow-slate-200/20">
              <div className="relative z-10">
                <div className="flex items-center justify-between mb-10">
                   <div className="text-[11px] font-black uppercase tracking-[0.4em] text-slate-300">Node Awareness</div>
                   <TrendingUp size={20} className="text-slate-100 group-hover:text-[var(--accent)] transition-colors" />
                </div>
                <div className="space-y-6">
                   {(data?.nodeAwareness || [
                     ...
                     <div key={i} className="flex items-center justify-between p-6 bg-[var(--background)]/50 rounded-[2rem] border border-slate-50 shadow-sm group/node hover:bg-[var(--surface)] hover:border-[var(--accent)] transition-all cursor-pointer">
              ...
              <div className="absolute -bottom-20 -right-20 w-64 h-64 bg-[var(--background)] rounded-full blur-[80px] opacity-0 group-hover:opacity-100 transition-opacity duration-700"></div>
           </div>
        </div>
      </div>
    </div>
