  ...
  return (
    <div className="max-w-[1400px] mx-auto animate-fade-in pb-20 px-4">
      <PremiumHeader 
        ...
        primaryAction={
          <div className="flex items-center gap-4">
             <button className="text-[10px] font-black text-slate-300 uppercase tracking-widest hover:text-[var(--text)] transition-colors">Acknowledge All</button>
             <button className="h-14 w-14 bg-[var(--surface)] border border-[var(--border)] rounded-2xl flex items-center justify-center text-slate-400 hover:text-[var(--text)] transition-all shadow-xl shadow-slate-200/20">
                <Settings size={20} />
             </button>
          </div>
        }
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12 animate-fade-up">
        {[
          { label: 'Platform Status', value: stats ? (stats.securityAlerts > 0 ? 'Degraded' : 'Optimal') : 'Optimal', icon: <CheckCircle2 size={20} />, color: (stats?.securityAlerts ?? 0) > 0 ? 'text-amber-500' : 'text-emerald-500', bg: 'bg-[var(--surface)]' },
          { label: 'System Load', value: `${stats?.resourceUsage ?? 24}%`, icon: <Activity size={20} />, color: 'text-indigo-500', bg: 'bg-[var(--surface)]' },
          { label: 'Global Uptime', value: '99.99%', icon: <Zap size={20} />, color: 'text-[var(--accent)]', bg: 'bg-[var(--surface)]' },
          { label: 'Active Audits', value: displayAlerts.length.toString(), icon: <Bell size={20} />, color: 'text-rose-500', bg: 'bg-[var(--surface)]' },
        ].map((kpi, i) => (
          <div key={i} className={`bg-[var(--surface)] border border-slate-50 p-8 rounded-[2.5rem] shadow-xl shadow-slate-200/20 flex items-center gap-6 group hover:shadow-2xl transition-all`}>
            <div className={`w-16 h-16 bg-[var(--background)] ${kpi.color} rounded-2xl flex items-center justify-center shadow-inner group-hover:rotate-6 transition-transform`}>
               {kpi.icon}
            </div>
            <div>
              <div className="text-[10px] font-black uppercase tracking-widest text-slate-300 mb-1">{kpi.label}</div>
              <div className="text-2xl font-serif font-black text-[var(--text)] tracking-tight leading-none">{kpi.value}</div>
            </div>
          </div>
        ))}
      </div>
      ...
      <div className="flex items-center justify-between mb-8 px-4 animate-fade-up">
         <div className="flex items-center gap-4">
            <h2 className="text-[10px] font-black text-[var(--text)] uppercase tracking-[0.4em]">Live Alerts</h2>
            <div className="h-px w-20 bg-slate-100"></div>
         </div>
         {isLoading && (
            <div className="flex items-center gap-3 text-[9px] font-black text-[var(--accent)] uppercase tracking-widest animate-pulse">
      ...
      <div className="space-y-6 animate-fade-up">
        {displayAlerts.map((alert: SystemAlert) => (
          <div key={alert.id} className="bg-[var(--surface)] rounded-[3rem] border border-slate-50 overflow-hidden shadow-xl shadow-slate-200/20 hover:shadow-2xl transition-all relative group">
            <div className={`absolute left-0 top-0 h-full w-2 ${
              ...
              alert.severity === 'MEDIUM' ? 'bg-[var(--accent)]' : 'bg-emerald-500'
            }`}></div>
            <div className="p-10 flex flex-col md:flex-row gap-10 items-center">
              <div className="w-20 h-20 bg-[var(--background)] rounded-[1.8rem] flex items-center justify-center group-hover:scale-105 transition-transform shrink-0 border border-[var(--border)] shadow-inner">
                {alert.severity === 'CRITICAL' ? <ShieldAlert size={36} className="text-rose-500" /> :
                 ...
                 alert.severity === 'MEDIUM' ? <Zap size={36} className="text-[var(--accent)]" /> :
              ...
              <div className="flex-1 text-center md:text-left">
                <div className="flex flex-col md:flex-row items-center justify-between mb-4 gap-4">
                  <div className="flex items-center gap-6">
                    <span className={`px-4 py-1.5 rounded-xl text-[9px] font-black uppercase tracking-[0.2em] border transition-all ${
                       ...
                       alert.severity === 'HIGH' ? 'bg-amber-50 text-amber-500 border-amber-100' : 'bg-[var(--accent)]/5 text-[var(--accent)] border-[var(--accent)]/10'
                    }`}>{alert.severity} Threshold</span>
                    <h3 className="text-2xl font-serif font-black text-[var(--text)] tracking-tight italic">{alert.message}</h3>
                  </div>
                ...
                <div className="flex flex-col md:flex-row items-center justify-between pt-6 border-t border-slate-50 gap-6">
                   <div className="flex items-center gap-6">
                      ...
                      <button className="text-[10px] font-black text-slate-300 hover:text-[var(--accent)] uppercase tracking-widest transition-colors">Acknowledge Alert</button>
                   </div>
