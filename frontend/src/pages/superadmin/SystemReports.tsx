    ...
    <div className="max-w-[1400px] mx-auto animate-fade-in pb-20 px-4">
      <PremiumHeader 
        ...
        primaryAction={
          ...
          >
            Generate New Report <Plus size={20} className="text-[var(--accent)]" />
          </button>
        }
      />
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-8 mb-16 animate-fade-up">
        {[
          { label: 'Platform Reports', value: reports.length.toString(), icon: <FileText size={24} />, color: 'bg-[var(--surface)]' },
          { label: 'Total Partners', value: stats?.totalPartners?.toString() || '85', icon: <Users size={24} />, color: 'bg-[var(--surface)]' },
          { label: 'System Load', value: `${stats?.resourceUsage || 24}%`, icon: <TrendingUp size={24} />, color: 'bg-[var(--surface)]' },
        ].map((kpi, i) => (
          <div key={i} className="bg-[var(--surface)] border border-slate-50 p-10 rounded-[2.5rem] shadow-xl shadow-slate-200/20 flex flex-col group hover:shadow-2xl transition-all">
            <div className={`w-14 h-14 bg-[var(--background)] text-[var(--accent)] rounded-2xl flex items-center justify-center mb-10 shadow-inner group-hover:scale-110 transition-transform`}>
              {kpi.icon}
            </div>
            <div className="text-[10px] font-black uppercase tracking-[0.2em] text-slate-300 mb-2">{kpi.label}</div>
            <div className="text-4xl font-serif font-black text-[var(--text)] tracking-tight leading-none">{kpi.value}</div>
          </div>
        ))}
      </div>
        ...
        <div className="lg:col-span-8 space-y-6 animate-fade-up delay-1">
           <div className="flex items-center justify-between mb-8 px-4">
              <div className="flex items-center gap-4">
                <h3 className="text-[10px] font-black uppercase tracking-[0.4em] text-[var(--text)]">Recent Reporting Cycle</h3>
                <div className="h-px w-20 bg-slate-100"></div>
              </div>
              {isLoading && <RefreshCw size={14} className="animate-spin text-[var(--accent)]" />}
              <button className="text-[9px] font-black text-slate-300 hover:text-[var(--text)] uppercase tracking-widest transition-colors">Archive Access</button>
           </div>
           
           <div className="space-y-4">
             {reports.length > 0 ? reports.map((report, idx) => (
               <div key={report.id} className="bg-[var(--surface)] rounded-[2.5rem] border border-slate-50 p-8 flex flex-col md:flex-row md:items-center justify-between gap-10 hover:shadow-2xl hover:shadow-slate-200/30 transition-all group animate-fade-up" style={{ animationDelay: `${idx * 0.05}s` }}>
                 <div className="flex items-center gap-8">
                   <div className={`w-20 h-20 rounded-[1.8rem] flex items-center justify-center transition-all shadow-inner border group-hover:scale-105 duration-500 ${
                     report.status === 'READY' ? 'bg-[var(--background)] border-[var(--border)] text-[var(--accent)] group-hover:bg-slate-900 group-hover:text-[var(--accent)]' : 
                   ...
                   <div>
                     <div className="flex flex-col md:flex-row md:items-center gap-4 mb-2">
                       <h4 className="text-xl font-bold text-[var(--text)] tracking-tight group-hover:text-[var(--accent)] transition-colors">{report.title}</h4>
                       <span className={`px-3 py-1 rounded-lg text-[9px] font-black uppercase tracking-widest border transition-all ${
                         ...
                         'bg-[var(--background)] text-slate-400 border-[var(--border)] group-hover:bg-slate-900 group-hover:text-white group-hover:border-slate-800'
                       }`}>{report.type}</span>
                     </div>
                     <div className="flex items-center gap-4 text-[10px] font-mono font-black text-slate-200 uppercase tracking-tighter">
                        <span className="text-slate-400 group-hover:text-[var(--text)] transition-colors">By: {report.generatedBy}</span>
                 ...
                 <div className="flex items-center gap-4 self-end md:self-center">
                    <div className="text-right mr-6 hidden md:block">
                        ...
                        <div className="text-[10px] font-mono font-black text-slate-300 uppercase tracking-tighter">{report.format} · <span className="text-slate-100 group-hover:text-[var(--accent)] transition-colors">{report.size}</span></div>
                    </div>
                    {report.status === 'READY' && (
                      <button className="h-14 px-8 bg-slate-900 text-white rounded-2xl flex items-center gap-3 text-[10px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-black transition-all">
                        <Download size={20} className="text-[var(--accent)]" /> Download
                      </button>
                    )}
                    <button 
                      onClick={() => handleDelete(report.id)}
                      className="w-14 h-14 rounded-2xl bg-[var(--surface)] border border-[var(--border)] text-slate-200 hover:text-rose-600 hover:bg-rose-50 transition-all shadow-sm"
                      title="Purge Record"
                    >
                      <X size={24} />
                    </button>
        ...
        <div className="lg:col-span-4 space-y-8 animate-fade-up delay-2">
            <div className="bg-slate-900 text-white rounded-[3.5rem] p-12 shadow-2xl relative overflow-hidden group">
               <div className="relative z-10">
                 <h3 className="font-serif text-3xl mb-8 leading-tight italic">Insight <span className="text-[var(--accent)] not-italic opacity-40">Generator</span></h3>
                 ...
                 <div className="space-y-6 mb-16">
                    {['Placement Efficiency', 'Skill Gap Delta', 'Institution Compliance', 'Corporate ROI Index'].map((item, i) => (
                      <div key={i} className="flex items-center gap-6 group/item cursor-pointer">
                         <div className="w-1.5 h-1.5 rounded-full bg-[var(--accent)] opacity-40 group-hover/item:opacity-100 transition-all shadow-glow"></div>
                         ...
                         <ArrowUpRight size={16} className="ml-auto text-white/10 group-hover/item:text-[var(--accent)] group-hover/item:translate-x-1 group-hover/item:-translate-y-1 transition-all" />
                      </div>
                    ))}
                 </div>
                 ...
                 <button 
                    onClick={() => setIsModalOpen(true)}
                    className="w-full h-16 bg-[var(--accent)] text-[var(--text)] rounded-[1.5rem] text-[11px] font-black uppercase tracking-[0.3em] transition-all hover:bg-teal-400 shadow-xl shadow-teal-500/20"
                 >
                    Initialize Synthesis
                 </button>
               </div>
               <div className="absolute top-0 right-0 p-12 opacity-5 scale-150 rotate-12">
                  <FileBox size={200} className="text-[var(--accent)]" />
               </div>
            </div>

            <div className="bg-[var(--surface)] border-2 border-dashed border-[var(--border)] rounded-[3rem] p-12 group relative overflow-hidden">
               <div className="relative z-10">
                 <div className="flex items-center justify-between mb-10">
                   <h4 className="text-[10px] font-black uppercase tracking-[0.4em] text-slate-300">Automation Hub</h4>
                   <RefreshCw size={18} className="text-slate-100 group-hover:text-[var(--accent)] group-hover:animate-spin-slow transition-all" />
                 </div>
                 <div className="flex items-center gap-4 mb-6">
                    <div className="w-2.5 h-2.5 bg-emerald-500 rounded-full animate-pulse shadow-glow"></div>
                    <div className="text-[11px] font-black text-[var(--text)] uppercase tracking-widest">Sentinel Core: Optimal</div>
                 </div>
                 ...
                 <div className="flex gap-4">
                    <button className="h-12 px-6 bg-[var(--background)] border border-[var(--border)] rounded-xl text-slate-200 hover:text-[var(--text)] hover:bg-[var(--surface)] transition-all"><Printer size={20} /></button>
                    <button className="h-12 px-6 bg-[var(--background)] border border-[var(--border)] rounded-xl text-slate-200 hover:text-[var(--text)] hover:bg-[var(--surface)] transition-all"><Share2 size={20} /></button>
                 </div>
               </div>
            </div>
        </div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-slate-900/60 backdrop-blur-2xl animate-fade-in">
          <div className="bg-[var(--surface)] rounded-[3.5rem] w-full max-w-xl overflow-hidden shadow-[0_60px_150px_rgba(13,148,136,0.3)] animate-scale-in relative border border-[var(--border)]">
            <div className="absolute top-0 left-0 w-full h-[6px] bg-[var(--accent)]"></div>
            
            <div className="p-16">
              <div className="flex items-center justify-between mb-12">
                <div>
                  <h3 className="text-3xl font-serif font-black text-[var(--text)] italic leading-none">Report <span className="text-slate-400 not-italic">Generation</span></h3>
                  <p className="text-[10px] font-black text-slate-300 uppercase tracking-widest mt-2">Create New Platform Report</p>
                </div>
                <button onClick={() => setIsModalOpen(false)} className="w-14 h-14 rounded-2xl bg-[var(--background)] flex items-center justify-center text-slate-300 hover:bg-rose-50 hover:text-rose-600 transition-all">
                  <X size={28} />
                </button>
              </div>
              ...
              <form onSubmit={handleGenerate} className="space-y-8">
                <div className="space-y-3">
                  ...
                  <input 
                    ...
                    className="w-full h-16 bg-[var(--background)]/50 border border-[var(--border)] rounded-2xl px-6 text-sm font-medium outline-none focus:ring-1 focus:ring-[var(--accent)] focus:bg-[var(--surface)] transition-all shadow-inner" 
                  />
                </div>
                ...
                <div className="space-y-3">
                  ...
                  <div className="relative">
                    <select 
                      ...
                      className="w-full h-16 bg-[var(--background)]/50 border border-[var(--border)] rounded-2xl px-6 text-[10px] font-black uppercase tracking-widest outline-none focus:ring-1 focus:ring-[var(--accent)] focus:bg-[var(--surface)] transition-all appearance-none cursor-pointer"
                    >
                ...
                <div className="p-8 bg-[var(--background)]/50 rounded-[2rem] flex items-start gap-5 border border-[var(--border)] shadow-inner">
                  <AlertCircle size={24} className="text-[var(--accent)] shrink-0 mt-0.5" />
