...
const Placements: React.FC = () => {
  ...
  const renderStatusBadge = (status: string) => {
    switch (status.toUpperCase()) {
      ...
      default:
        return (
          <span className="px-5 py-2 bg-[var(--background)] text-[var(--color-muted)] rounded-full text-[10px] font-black uppercase tracking-widest border border-[var(--border)] flex items-center gap-1.5 shadow-sm">
  ...
  return (
    <div className="max-w-[1280px] mx-auto animate-fade-in pb-20 px-4">
      ...
      <div className="max-w-2xl mx-auto space-y-10 mt-4">
        ...
        <div className="sticky top-[70px] lg:top-[80px] z-50 -mx-4 px-4 py-2 bg-[var(--color-bg)]/80 backdrop-blur-xl border-b border-[var(--border)] lg:border-none lg:bg-transparent lg:p-0 lg:static">
          <div className="bg-[var(--surface)] lg:bg-[var(--surface)]/90 rounded-2xl lg:rounded-3xl border border-[var(--border)] p-1.5 flex lg:flex-row items-center shadow-sm overflow-x-auto scrollbar-hide flex-nowrap gap-1 lg:gap-0">
               {tabs.map((tab) => (
                  <button 
                    key={tab.id}
                    ...
                    className={`flex-none lg:flex-1 px-6 lg:px-0 py-3 text-[10px] lg:text-[11px] font-black uppercase tracking-widest rounded-xl lg:rounded-2xl transition-all relative group whitespace-nowrap ${
                      activeTab === tab.id ? 'bg-[var(--accent)] text-white shadow-lg shadow-indigo-500/20' : 'text-slate-400 hover:bg-[var(--background)]'
                    }`}
                  >
                     {tab.label}
                     {tab.count > 0 && (
                       <span className={`ml-2 inline-flex min-w-[18px] h-4 px-1 rounded-full items-center justify-center text-[8px] font-black border transition-all ${
                         activeTab === tab.id ? 'bg-[var(--surface)] text-[var(--accent)] border-white/20' : 'bg-[var(--background)] text-[var(--color-muted)] border-[var(--border)] group-hover:bg-[var(--surface)]'
                       }`}>
                          {tab.count}
                       </span>
                     )}
        ...
        <div className="relative group">
             <Search className="absolute left-5 top-1/2 -translate-y-1/2 text-slate-300 group-focus-within:text-[var(--accent)] transition-colors" size={20} />
             <input 
               type="text" 
               placeholder={`Search in ${tabs.find(t => t.id === activeTab)?.label}...`}
               value={searchQuery}
               onChange={(e) => setSearchQuery(e.target.value)}
               className="w-full h-14 lg:h-16 bg-[var(--surface)] border border-[var(--border)] rounded-2xl lg:rounded-[2rem] pl-14 pr-6 text-sm font-medium outline-none focus:ring-2 focus:ring-[var(--accent)]/10 focus:border-[var(--accent)] transition-all shadow-sm"
             />
        </div>
        ...
        <div className="space-y-6">
          {isLoading ? (
            <div className="space-y-6">
               {[1,2,3].map(i => (
                 <div key={i} className="h-64 bg-[var(--surface)] border border-[var(--border)] rounded-[2.5rem] animate-pulse"></div>
               ))}
            </div>
             ...
             <div className="space-y-6">
                {filteredData.map((item: InternshipDTO | ApplicationDTO, i: number) => (
                   <div 
                    key={item.id} 
                    className="bg-[var(--surface)] rounded-[2rem] lg:rounded-[2.5rem] border border-[var(--border)] p-6 lg:p-10 shadow-xl shadow-slate-200/30 hover:shadow-2xl transition-all group animate-fade-up relative overflow-hidden" 
                    style={{ animationDelay: `${i * 0.05}s` }}
                  >
                     <div className="flex flex-col lg:flex-row justify-between items-start gap-6 lg:gap-0">
                        <div className="flex flex-col lg:flex-row gap-4 lg:gap-8">
                           <div className="w-12 h-12 lg:w-16 lg:h-16 rounded-xl lg:rounded-2xl bg-[var(--color-brand-faint)] flex items-center justify-center text-[var(--accent)] font-bold text-xl lg:text-2xl shadow-inner group-hover:scale-110 transition-transform">
                             {('companyName' in item ? item.companyName : (item as { companyName?: string }).companyName)?.[0] || 'I'}
                           </div>
                           <div>
                              <h4 className="text-xl lg:text-2xl font-bold text-[var(--text)] group-hover:text-[var(--accent)] transition-colors mb-2 leading-tight">
                                 {'title' in item ? item.title : item.internshipTitle}
                              </h4>
                              <div className="flex items-center gap-2 mb-4">
                                <Briefcase size={14} className="text-slate-300" />
                                <span className="text-[10px] lg:text-sm font-bold text-[var(--color-muted)] uppercase tracking-widest">
                        ...
                        {activeTab === 'discover' && (
                           <button onClick={() => setApplyingTo(item as InternshipDTO)} className="hidden lg:block p-2 text-slate-200 hover:text-[var(--accent)] transition-all">
                              <Zap size={24} />
                           </button>
                        )}
                     </div>
                     ...
                     <div className="mt-6 lg:mt-8 pt-6 lg:pt-8 border-t border-slate-50 flex flex-col lg:flex-row items-center justify-between gap-4">
                        <div className="flex gap-2 w-full lg:w-auto overflow-x-auto scrollbar-hide pb-2 lg:pb-0">
                           {('requiredSkills' in item ? item.requiredSkills : 'Institutional Alignment').split(',').slice(0, 3).map((s: string, idx: number) => (
                             <span key={idx} className="px-3 py-1.5 lg:px-4 lg:py-1.5 bg-[var(--background)] text-[9px] lg:text-[10px] font-bold uppercase tracking-widest text-slate-400 rounded-xl border border-[var(--border)] whitespace-nowrap">
                                {s.trim()}
                             </span>
                           ))}
                        </div>
                        ...
                        {activeTab === 'discover' ? (
                           <button 
                             ...
                             className="w-full lg:w-auto px-8 py-3 bg-[var(--surface)] border-2 border-[var(--accent)] text-[var(--accent)] rounded-2xl text-[10px] lg:text-[11px] font-black uppercase tracking-[0.2em] hover:bg-[var(--accent)] hover:text-white transition-all active:scale-95 shadow-lg shadow-indigo-500/5"
                           >
                             Quick Apply
                           </button>
                        ) : (
            ...
            <div className="py-20 bg-[var(--surface)] rounded-[3rem] border border-dashed border-[var(--border)] text-center px-10">
                ...
                <h3 className="text-2xl font-serif font-bold text-[var(--text)] italic mb-2">No Active Records Found</h3>
      ...
      {applyingTo && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 sm:p-12 animate-fade-in">
          ...
          <div className="bg-[var(--surface)] rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl relative animate-scale-in border border-[var(--border)]/50">
            <div className="p-6 lg:p-10 pb-4 lg:pb-6 flex justify-between items-start">
              <div className="flex gap-4 lg:gap-6">
                <div className="w-12 h-12 lg:w-16 lg:h-16 rounded-xl lg:rounded-2xl bg-[var(--color-brand-faint)] flex items-center justify-center text-[var(--accent)] font-bold text-xl lg:text-2xl shadow-inner">
                  {applyingTo.companyName?.[0] || 'I'}{applyingTo.companyName?.[1] || ''}
                </div>
                <div>
                  <h3 className="text-xl lg:text-2xl font-bold text-[var(--text)] tracking-tight leading-tight mb-1">Apply for {applyingTo.title}</h3>
                  <p className="text-[9px] lg:text-[11px] text-[var(--color-muted)] font-black uppercase tracking-widest">{applyingTo.companyName} · Institutional Registry</p>
                </div>
              </div>
              <button onClick={() => setApplyingTo(null)} className="p-2 text-slate-300 hover:text-rose-500 transition-all rounded-xl hover:bg-[var(--background)]">
                <X size={24} />
              </button>
            </div>
            ...
            <form onSubmit={handleApply} className="p-6 lg:p-10 pt-0 space-y-6 lg:space-y-8">
              <div className="space-y-3">
                ...
                <textarea 
                  ...
                  rows={5}
                  className="w-full bg-[var(--background)] border border-[var(--border)] rounded-2xl lg:rounded-3xl p-5 lg:p-6 text-sm font-medium outline-none focus:ring-2 focus:ring-[var(--accent)]/10 focus:border-[var(--accent)] transition-all shadow-inner" 
                />
              </div>

              <div className="bg-[var(--color-brand-faint)] rounded-2xl lg:rounded-[1.5rem] p-4 lg:p-6 border border-indigo-100">
                 <div className="flex items-start gap-3 lg:gap-4">
                    <div className="w-8 h-8 lg:w-10 lg:h-10 rounded-lg lg:rounded-xl bg-[var(--surface)] flex items-center justify-center shadow-sm flex-none">
                       <Target size={20} className="text-[var(--accent)]" />
                    </div>
                    <div>
                       <div className="text-[10px] lg:text-[11px] font-black text-[var(--accent)] uppercase tracking-widest">Digital Authentication</div>
                       <p className="text-[10px] lg:text-[11px] text-[var(--color-muted)] font-medium mt-1">Your verified scholarship record will be transmitted instantly upon submission.</p>
                    </div>
                 </div>
              </div>
              ...
              <div className="flex flex-col lg:flex-row items-center gap-4">
                <button 
                  type="submit"
                  className="w-full lg:flex-1 h-14 bg-[var(--accent)] text-white text-[10px] lg:text-[11px] font-black uppercase tracking-[0.3em] rounded-2xl shadow-xl shadow-indigo-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-4"
                >
                ...
                <button 
                  ...
                  className="w-full lg:w-auto px-10 h-14 bg-[var(--surface)] border border-[var(--border)] text-slate-400 text-[10px] lg:text-[11px] font-black uppercase tracking-[0.3em] rounded-2xl hover:bg-[var(--background)] transition-all"
                >
                  Abort
                </button>
              </div>
