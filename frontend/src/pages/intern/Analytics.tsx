    ...
    <div className="max-w-[1240px] mx-auto animate-fade-in pb-20 mt-6 px-4">
      ...
      <div className="flex flex-col mb-10 px-2">
         ...
         <div className="flex justify-between items-end">
            <div>
               <h1 className="text-3xl font-black text-[var(--text)] tracking-tight mb-1">Performance Overview</h1>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-10">
        {stats.map((stat, i) => (
          <div key={i} className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-4 shadow-sm hover:shadow-fox transition-all group">
             <div className="w-8 h-8 rounded-lg bg-[var(--background)] flex items-center justify-center mb-4 text-slate-400 group-hover:text-[var(--text)] transition-colors">
                {stat.icon}
             </div>
             <div className="text-[9px] font-black text-slate-300 uppercase tracking-widest mb-1">{stat.label}</div>
             <div className="text-2xl font-black text-[var(--text)] tracking-tight mb-1">{stat.value}</div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
        ...
        <div className="lg:col-span-8 bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-8 relative overflow-hidden shadow-fox">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-6">
            <div className="flex items-center gap-4">
              <div className="h-[2px] w-6 bg-slate-200"></div>
              <div>
                <h3 className="text-lg font-black text-[var(--text)] tracking-tight">Activity Levels</h3>
            ...
            <div className="flex bg-[var(--background)] p-1 rounded-xl border border-[var(--border)]">
              {['Periodic', 'Cumulative', 'Forecast'].map((view, i) => (
                <button key={i} className={`px-4 py-2 rounded-lg text-[9px] font-black uppercase tracking-widest transition-all ${i === 0 ? 'bg-[var(--surface)] text-[var(--text)] shadow-sm border border-[var(--border)]' : 'text-slate-400 hover:text-[var(--color-muted)]'}`}>{view}</button>
              ))}
            </div>
          </div>
          ...
          <div className="h-64 flex items-end justify-between gap-4 px-2 pb-2 border-b border-slate-50 relative">
             ...
             <div className="absolute inset-0 flex flex-col justify-between pointer-events-none opacity-50 py-2">
                {[1,2,3].map(l => <div key={l} className="h-[1px] w-full bg-[var(--background)]"></div>)}
             </div>
          </div>
          ...
          <div className="mt-12 grid grid-cols-1 md:grid-cols-2 gap-6">
             <div className="flex items-center gap-4 p-4 bg-[var(--background)] border border-[var(--border)] rounded-xl">
                ...
                <div>
                   ...
                   <div className="text-[13px] font-bold text-[var(--text)]">Positive trajectory in skill acquisition.</div>
                </div>
             </div>
             <div className="flex items-center gap-4 p-4 bg-[var(--background)] border border-[var(--border)] rounded-xl">
                <div className="w-10 h-10 bg-[var(--surface)] border border-[var(--border)] rounded-xl flex items-center justify-center text-emerald-500 shadow-sm"><Zap size={18} /></div>
                <div>
                   <div className="text-[8px] font-black uppercase tracking-[0.2em] text-slate-400 mb-1">Cycle Milestone</div>
                   <div className="text-[13px] font-bold text-[var(--text)]">Peak throughput achieved.</div>
                </div>
             </div>
          </div>
        </div>
        ...
        <div className="lg:col-span-4 space-y-8">
           <div className="bg-slate-900 rounded-2xl p-6 text-white shadow-fox relative overflow-hidden group">
             <div className="relative z-10">
               ...
               <div className="space-y-6">
                 {[
                   ...
                   <div key={idx} className="group/skill">
                     ...
                     <div className="h-1 w-full bg-[var(--surface)]/5 rounded-full overflow-hidden">
                       <div 
               ...
               <div className="mt-10 p-5 bg-[var(--surface)]/5 rounded-xl border border-white/10 group-hover:bg-[var(--surface)]/10 transition-all shadow-lg">
                  <div className="flex items-center gap-4 mb-4">
                     <div className="w-10 h-10 bg-[var(--surface)]/5 rounded-lg flex items-center justify-center text-amber-400 border border-white/10">
                       <PieChart size={20} />
                     </div>
                     <div>
                  ...
                  <button className="w-full h-10 bg-amber-400 text-[var(--text)] rounded-lg text-[9px] font-black uppercase tracking-[0.2em] hover:scale-[1.02] active:scale-95 transition-all shadow-lg shadow-amber-500/10">Download Audit</button>
               </div>
             </div>
           </div>
           ...
           <div className="bg-[var(--surface)] border border-[var(--border)] rounded-2xl p-6 shadow-sm">
              <div className="flex items-center gap-3 mb-6">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-widest text-[var(--text)]">Success Rating</span>
              </div>
              <p className="text-[11px] text-[var(--color-muted)] leading-relaxed font-medium italic opacity-80">"Academic velocity demonstrates sustained excellence across all terminal benchmarks."</p>
           </div>
        </div>
      </div>
    </div>
