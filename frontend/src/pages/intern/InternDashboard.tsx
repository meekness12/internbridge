    ...
    <>
      ...
      {selectedJob && (
        <div className="fixed inset-0 z-[150] flex items-center justify-center p-6 sm:p-12 animate-fade-in">
           ...
           <div className="bg-[var(--surface)] rounded-[2.5rem] w-full max-w-2xl overflow-hidden shadow-2xl relative animate-scale-in max-h-[90vh] flex flex-col">
              <div className="p-8 pb-4 flex justify-between items-start">
                 <div className="flex gap-6">
                    <div className="w-20 h-20 rounded-[1.5rem] bg-[var(--color-teal-faint)] flex items-center justify-center text-[var(--accent)] font-bold text-2xl shadow-inner">
                       {selectedJob.companyName?.[0] || 'I'}{selectedJob.companyName?.[1] || ''}
                    </div>
                    <div className="pt-1">
                       <h2 className="text-3xl font-bold text-[var(--text)] tracking-tight leading-none mb-2">{selectedJob.title}</h2>
                       <div className="flex items-center gap-2">
                         <Briefcase size={16} className="text-slate-300" />
                         <span className="text-sm font-bold text-[var(--color-muted)] uppercase tracking-widest">{selectedJob.companyName}</span>
                       </div>
                    </div>
                 </div>
                 <button onClick={() => setSelectedJob(null)} className="p-2 rounded-xl hover:bg-[var(--background)] transition-colors text-slate-400">
                    <X size={24} />
                 </button>
              </div>
                 ...
                 <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                    {[
                      ...
                      <div key={i} className={`${m.bg} p-3 rounded-2xl flex items-center gap-3 border border-[var(--border)]/50 shadow-sm`}>
                         ...
                         <span className="text-[11px] font-bold text-[var(--color-muted)] truncate">{m.label}</span>
                      </div>
                    ))}
                 </div>
              ...
              {/* Action Buttons */}
              <div className="p-8 border-t border-slate-50 bg-[var(--background)]/50 flex gap-4">
                 <button 
                   onClick={() => handleApply(selectedJob.id)}
                   className="flex-1 h-14 bg-[var(--accent)] text-white text-[11px] font-black uppercase tracking-[0.3em] rounded-2xl shadow-xl shadow-teal-500/20 hover:scale-[1.02] active:scale-[0.98] transition-all"
                 >
                   Apply Now
                 </button>
                 <button 
                   onClick={() => setSelectedJob(null)}
                   className="px-10 h-14 bg-[var(--surface)] border border-[var(--border)] text-[var(--color-muted)] text-[11px] font-black uppercase tracking-[0.3em] rounded-2xl hover:bg-[var(--background)] transition-all shadow-sm"
                 >
                   Close
                 </button>
              </div>
